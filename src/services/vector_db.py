...
class VectorDB:
    ...
    def __init__(self, persist_directory: str='./chroma_db'):
        ...
        if config.chromadb_host and config.chromadb_host != 'localhost':
            self.client = chromadb.HttpClient(host=config.chromadb_host, port=config.chromadb_port, settings=Settings(anonymized_telemetry=False, allow_reset=True))
        else:
        ...
        try:
            ...
            self.embedding_function = embedding_functions.SentenceTransformerEmbeddingFunction(model_name='truro7/vn-law-embedding')
    ...
    def create_collection(self, collection_name: str, reset: bool=False):
        if reset:
            try:
                ...
                pass
        return self.client.get_or_create_collection(name=collection_name, metadata={'hnsw:space': 'cosine'}, embedding_function=self.embedding_function)

    def add_documents(self, collection_name: str, documents: List[str], metadatas: List[Dict], ids: Optional[List[str]]=None):
...
if __name__ == '__main__':
    ...
    documents = ['Chính sách nghỉ phép: 12 ngày/năm', 'Thời gian làm việc: 8:30 - 17:30', 'Lương: Trả vào ngày 10 hàng tháng']
    metadatas = [{'question': 'Nghỉ phép bao nhiêu ngày?', 'answer': '12 ngày/năm', 'source': 'hr_policy.txt'}, {'question': 'Giờ làm việc?', 'answer': '8:30 - 17:30', 'source': 'hr_policy.txt'}, {'question': 'Khi nào trả lương?', 'answer': 'Ngày 10 hàng tháng', 'source': 'hr_policy.txt'}]
