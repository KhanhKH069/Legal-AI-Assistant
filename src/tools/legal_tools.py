"""
Legal Tools — Công cụ tra cứu Pháp điển và Án lệ

Hai tool chính cho Legal AI Agent:
  - search_statutory_law : Tìm kiếm điều luật trong Pháp điển (ChromaDB: legal_statutory)
  - search_case_law      : Tìm kiếm án lệ, bản án (ChromaDB: legal_caselaw)
  - search_legal_qa      : Tìm kiếm các câu hỏi đáp pháp luật (ChromaDB: legal_qa)
"""
...
def _get_statutory_retriever():
    """Lazy singleton for statutory law retriever."""
    return get_hybrid_retriever("legal_statutory")


def _get_caselaw_retriever():
    """Lazy singleton for case law retriever."""
...
def search_statutory_law(query: str, top_k: int = 5) -> str:
    """
    Tìm kiếm điều luật, quy định pháp luật trong Hệ thống Pháp điển Quốc gia Việt Nam.

    Dùng khi người dùng hỏi về:
    - Quy định pháp luật cụ thể (luật dân sự, hình sự, lao động, doanh nghiệp...)
    - Nghĩa vụ, quyền lợi theo pháp luật
    - Thủ tục hành chính, pháp lý
    - Chế tài, hình phạt theo luật

    Args:
        query: Câu hỏi hoặc từ khóa cần tra cứu pháp luật
        top_k: Số lượng điều luật trả về (mặc định 5)

    Returns:
        Các điều luật liên quan với trích dẫn nguồn
    """
    try:
...
def search_case_law(query: str, top_k: int = 5) -> str:
    """
    Tìm kiếm án lệ và bản án của Tòa án nhân dân Việt Nam.

    Dùng khi người dùng hỏi về:
    - Án lệ áp dụng trong tình huống tranh chấp cụ thể
    - Cách Tòa án giải quyết các loại vụ án
    - Tiền lệ pháp lý cho một vấn đề pháp lý
    - Quyết định giám đốc thẩm, tái thẩm

    Args:
        query: Câu hỏi về tình huống tranh chấp hoặc loại án cần tìm
        top_k: Số lượng án lệ trả về (mặc định 5)

    Returns:
        Các án lệ, bản án liên quan với trích dẫn nguồn
    """
    try:
