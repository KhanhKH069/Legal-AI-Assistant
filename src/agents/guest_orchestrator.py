...
def create_guest_orchestrator():
    prompt = ChatPromptTemplate.from_messages([('system', _GUEST_ORCHESTRATOR_PROMPT), MessagesPlaceholder(variable_name='messages')])
...
def guest_orchestrator_node(state: GuestLegalState):
    ...
    return {'messages': state['messages'], 'next': next_agent, 'user_intent': upper, 'session_id': state.get('session_id', 'guest')}
...
def create_guest_agent_graph():
    ...
    def guest_statutory_node(state: GuestLegalState):
        ...
        adapted: AgentState = {'messages': state['messages'], 'next': '', 'user_intent': state.get('user_intent', ''), 'user_id': state.get('session_id', 'guest'), 'user_info': {'role': 'guest'}}
        result = statutory_agent_node(adapted)
        return {'messages': result.get('messages', []), 'next': 'end', 'user_intent': state.get('user_intent', ''), 'session_id': state.get('session_id', 'guest')}
    ...
    workflow.add_conditional_edges('guest_orchestrator', guest_router, {'statutory_agent': 'statutory_agent', 'caselaw_agent': 'caselaw_agent'})
    ...
    def route_caselaw(state):
        ...
        return 'end'
    workflow.add_conditional_edges('statutory_agent', route_statutory, {'statutory_tools': 'statutory_tools', 'end': END})
    workflow.add_edge('statutory_tools', 'statutory_agent')
    workflow.add_conditional_edges('caselaw_agent', route_caselaw, {'caselaw_tools': 'caselaw_tools', 'end': END})
