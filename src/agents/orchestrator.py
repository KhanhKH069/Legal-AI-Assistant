...
def create_orchestrator():
    prompt = ChatPromptTemplate.from_messages([('system', _ORCHESTRATOR_PROMPT), MessagesPlaceholder(variable_name='messages')])
...
def orchestrator_node(state: AgentState):
    ...
    return {'messages': state['messages'], 'next': next_agent, 'user_intent': response_clean, 'user_id': state.get('user_id', ''), 'user_info': state.get('user_info', {}), 'retry_count': 0}
...
def create_legal_agent_graph():
    ...
    workflow.add_conditional_edges('orchestrator', router, {'statutory_agent': 'statutory_agent', 'caselaw_agent': 'caselaw_agent', 'end': END})
    ...
    def route_reviewer(state: AgentState) -> str:
        if state.get('next') == 'fail':
            return 'statutory_agent'
        return 'end'
    workflow.add_conditional_edges('statutory_agent', route_statutory, {'statutory_tools': 'statutory_tools', 'reviewer_node': 'reviewer_node'})
    workflow.add_edge('statutory_tools', 'statutory_agent')
    workflow.add_conditional_edges('reviewer_node', route_reviewer, {'statutory_agent': 'statutory_agent', 'end': END})
