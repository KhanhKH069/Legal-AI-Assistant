...
class APIGateway:
    ...
    def __init__(self, audit_logger):
        ...
        self.authenticated_users = {'user001': {'role': 'employee', 'name': 'Nguyễn Văn A'}, 'user002': {'role': 'hr', 'name': 'HR Manager'}, 'admin': {'role': 'admin', 'name': 'Admin'}}
    ...
    def check_rate_limit(self, user_id: str) -> bool:
        ...
        self.rate_limits[user_id] = [ts for ts in self.rate_limits[user_id] if now - ts < 60]
    ...
    def process_request(self, user_id: str, message: str, api_key: str=None) -> dict:
        ...
        return {'status': 'success', 'user_id': user_id, 'user_info': self.authenticated_users[user_id], 'message': message}