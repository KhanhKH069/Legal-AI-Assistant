"""Configuration Management — Local LLM (Qwen2.5 via Ollama)
"""
...
class Config:
    """Application configuration for Legal AI Assistant (Local LLMs)"""
    ...
    def validate(self) -> bool:
        ...
        if not self.llm_api_base and not self.enable_offline_mode:
            raise ValueError(
                "OPENAI_API_BASE not set in environment and offline mode is disabled"
            )
        return True
