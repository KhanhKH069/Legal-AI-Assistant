...
def main():
    ...
    with open('data/raw/legal_qa.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    collection = client.get_or_create_collection(name='legal_qa', embedding_function=emb_fn)
    ids = []
    documents = []
    metadatas = []
    for item in data:
        ...
        metadatas.append({'category': item.get('category', ''), 'source': item.get('source', ''), 'question': item['question']})
