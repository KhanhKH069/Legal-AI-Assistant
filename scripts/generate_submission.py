...
def generate_submission(input_file: str, output_file: str):
    ...
    for item in tqdm(data, desc='Evaluating'):
        ...
        try:
            response: SubmissionResponse = llm.invoke([SystemMessage(content=system_prompt), HumanMessage(content=prompt)])
            result_item = {'id': question_id, 'question': question, 'answer': response.answer, 'relevant_docs': response.relevant_docs, 'relevant_articles': response.relevant_articles}
        except Exception as e:
            print(f'Error processing question {question_id}: {e}')
            result_item = {'id': question_id, 'question': question, 'answer': 'Hệ thống gặp lỗi trong quá trình xử lý.', 'relevant_docs': [], 'relevant_articles': []}
